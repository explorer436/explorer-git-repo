...
***** Delete unnecessary spaces and tabs in empty lines in Vim
     ...
     (This doesn't seem to work with eclipse vim plugin and emacs vi mode)
...
***** Delete whitespace in blank lines
       ...
       search lines that only have whitespaces and replace with an empty string.
...
***** Delete all text before search pattern
   ...
   If you want to delete the text before the pattern ":" (colon). I have used the following command to replace every 
   ...
   I remember them as z == zero-width (since they don't match anything).
...
***** Delete all blank(empty) lines
   ...
   To delete blank lines in vim (empty lines), use this command - 
...
***** Delete all lines that do NOT contain a certain word in Vim
      ...
      This deletes every line that doesn't contain the word "price"
