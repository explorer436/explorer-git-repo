...
public class GenomicRangeQuery
{
    ...
    /*public static int[] solution(String S, int[] P, int[] Q)
    {
    		...
    		int[] S_int_values_list = new int[S.length()];
    		for(int i = 0; i < S.length(); i++)
    		{
    			switch (S.charAt(i)) 
    	        {
    	            case 'A':  S_int_values_list[i] = 1; break;
    	            case 'C':  S_int_values_list[i] = 2; break;
    	            case 'G':  S_int_values_list[i] = 3; break;
    	            case 'T':  S_int_values_list[i] = 4; break;
    	        }
    		}
        ...
        for(int i = 0; i < P.length; i++)
        {
        		...
        		if(pth == qth)
        		{
        			result[i] = S_int_values_list[pth];
        		}
        		else if(qth > pth)
        		{
        			int min = S_int_values_list[pth];
        			for(int j = pth; j <= qth; j++)
        			{
        				if (S_int_values_list[j] < min) 
        			    {
        			        min = S_int_values_list[j];
        			    }
        			}
