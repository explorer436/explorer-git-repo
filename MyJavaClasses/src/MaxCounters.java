public class MaxCounters
{
    ...
    public static int[] solution(int N, int[] A)
    {
        ...
        for (int K = 0; K < A.length; K++)
        {
            ...
            if (1 <= currentArrayValue && currentArrayValue <= N)
            {
                ...
                System.out.println("counter" + (currentArrayValue - 1) + " : " + c_num + " - min_counter_value : " + min_counter_value);   
    ...
    /*public static int[] solution(int N, int[] A)
    {
        ...
        for(int K=0; K<A.length; K++)
        {
            ...
            if(1 <= A[K] && A[K] <= N)
            {                                
                counterArray[A[K] - 1] = increase(counterArray[A[K] - 1]);
            }
            else if(A[K] == N + 1)
            {                
                //get max counter and put it in every location of counterArray
                int maxCounterVal = getMaxCounterValue(counterArray);
                //System.out.println("maxCounterVal : " + maxCounterVal);
