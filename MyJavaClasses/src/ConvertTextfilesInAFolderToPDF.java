...
public class ConvertTextfilesInAFolderToPDF {
    ...
    private static void readFileNames(String folderLocation){
        ...
        for (int i = 0; i < listOfFiles.length; i++) {
          ...
          if (listOfFiles[i].isFile()) 
          {
            ...
            if(listOfFiles[i].getName().substring(listOfFiles[i].getName().length()-4, listOfFiles[i].getName().length()).equals(".txt")){
                ...
                String pdfFN = folderLocation+"/"+listOfFiles[i].getName().substring(0, listOfFiles[i].getName().length()-4)+".pdf";
    ...
    try {
      input = 
        new BufferedReader (new FileReader(txtFileName));
      // letter 8.5x11
