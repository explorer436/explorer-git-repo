...
public class CookieManager {
	...
	for (int i=1; (headerName = conn.getHeaderFieldKey(i)) != null; i++) {
		...
		while (st.hasMoreTokens()) {
		    String token  = st.nextToken();
                    ...
                    if(-1 != token.indexOf(NAME_VALUE_SEPARATOR))
                    {
                        System.out.println("token.indexOf(NAME_VALUE_SEPARATOR) : " + token.indexOf(NAME_VALUE_SEPARATOR));
                        String a = token.substring(0, token.indexOf(NAME_VALUE_SEPARATOR)).toLowerCase();
                        System.out.println("a : " + a);
                        String b = token.substring(token.indexOf(NAME_VALUE_SEPARATOR) + 1, token.length());
	...
	try {
	    // URL url = new URL("http://www.google.com");
            ...
            URL url = new URL("http://gowatchit.com/movies/elizabeth-42260");
	    URLConnection conn = url.openConnection();
	    conn.connect();
	    cm.storeCookies(conn);
